...
export const Income = () => {
  ...
  const { address, transactions, usdcBalance, gigBalance, updateBalance } = useWalletStore();
  ...
  useEffect(() => {
    const updateGasEstimate = async () => {
      ...
      try {
        const estimate = await contracts.estimateTradeGas(isBuying, tradeAmount);
  ...
  const handleTrade = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTradeError(error instanceof Error ? error.message : 'Trade failed. Please try again.');
    } finally {
      setIsTrading(false);
    }
  };
